...
- (IBAction)textFieldDidChange:(UITextField *)textField {
    ...
    //密码登录
    if ([NSString validateMobile:self.phoneTF.text] && ![NSString isBlankString:self.passwordTF.text]) {
        self.loginBtn.enabled = YES;
        self.loginBtn.backgroundColor = UIColorFromRGB(0xFF6D22);
    }else{
        self.loginBtn.enabled = NO;
        self.loginBtn.backgroundColor = UIColorFromRGB(0xF6C8AC);
    }
}
...
- (WFLoginBottomView *)bottomView {
    if (!_bottomView) {
        ...
        _bottomView.lrType = WFRegisterType;
        @weakify(self)
